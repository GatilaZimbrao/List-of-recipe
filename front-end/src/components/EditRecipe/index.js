...
export default function EditRecipeComponent(props) {
  ...
  const editRecipe = async (body) => {
    RecipeService.editRecipe(body, props.id).then(() => {
      NavToRecipeList();
    });
  };
  ...
  return (
    <div
      className={"editRecipe d-flex justify-content-center align-items-center"}
      style={{ padding: "25px", paddingBottom: "75px", minHeight: "86.7vh" }}
    >
      <Form noValidate validated={validated} onSubmit={handleEditRecipe}>
        <h1 className="editRecipe--title text-center ">Editar Receita</h1>
        <Row>
          <Col lg={12}>
            <Form.Group>
